...
const arr_contactInfo=[
    {id:'location_info', img:'./assets/img/svg/location-icon.svg', header:'Pay Us a Visit', text:'Union St, Seattle, WA 98101, United States', link:'https://maps.app.goo.gl/q2TqyJBdPy1E8Pnu7', alt:'location icon' },
    {id:'phone_info', img:'./assets/img/svg/phone-call.svg', header:'Give Us a Call', text:'(110) 1111-1010', link:'tel:11011111010', alt:'phone icon' },
    {id:'email_info', img:'./assets/img/svg/mail.svg', header:'Send Us a Message', text:'Contact@HydraVTech.com', link:'mailto:Contact@HydraVTech.com', alt:'email icon' },    
]
const id_contactInfo=document.querySelector("#contactInfo_small");
if(id_contactInfo.childElementCount==0){
    id_contactInfo.innerHTML=carouselTemplate("change_contactInfo",tmpl_contactInfo,arr_contactInfo);
}
...
document.querySelector("#contactInfo_large").innerHTML =arr_contactInfo.map((info,i)=>{
    if(arr_contactInfo.length-1===i) return fillInfoItem(info)
    else{
        return `${fillInfoItem(info)} <img src="${vertical_line}" alt="vertical line">`
    }
}).join("");
...
// #region serviceInfo
    const arr_serviceInfo=[
        { img:'./assets/img/pexels-shvets-production-card-1.png', header:'SIMULATION' },
        { img:'./assets/img/pexels-mikhail-nilov-card-2.png', header:'EDUCATION' },
        { img:'./assets/img/pexels-rodnae-productions-card-3.png', header:'SELF-CARE' },
    ...
    if(id_serviceInfo.childElementCount==0){
        id_serviceInfo.innerHTML=carouselTemplate("change_serviceInfo",tmpl_serviceInfo,arr_serviceInfo);
...
if(id_techInfo.childElementCount==0){
    id_techInfo.innerHTML=carouselTemplate("change_techInfo",tmpl_techInfo,arr_techInfo);
}
...
if(id_processInfo.childElementCount==0){
    id_processInfo.innerHTML=carouselTemplate("change_processInfo",tmpl_processInfo,arr_processInfo);
}
...
function fillItem(index,actualElement,templateFn,arr){
    const html=`<div class="carousel__item" index="${index}">${templateFn(arr[index])}</div>`
    actualElement.outerHTML=html;
}
function chevronTemplate(onclickFn,isRight){
    return `
    <div class="chevron chevron-${isRight?"right":"left"}"  onclick="${onclickFn}(this,${isRight})">
...
function carouselTemplate(fnChange,templateFn,arr,index=0){
    ...
    <div class="carousel__box">
        ...
        <div class="carousel__item" index="${index}">${templateFn(arr[index])}</div>
...
document.querySelector('#join_form').addEventListener('submit', function(event) {
    event.preventDefault()
    const requiredValues=["email","firstName","lastName","phone"]
    let status=true;
    for(let item of document.querySelector('#join_form').querySelectorAll('input, textarea')){
        if(item?.id==="email"){
